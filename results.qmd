...
pre_network <- st_read(eisenberg_connection, "porto_alegre_net_largest")
```
...
The area of interested  covered `r round(st_area(st_as_sfc(st_bbox(ghs$geom)))/1000000)` km² of surface including `r nrow(nuts_porto)` municipalities with `r nrow(hospitals)` healthcare facilities. The size of the network where the centrality analysis was conducted contained `r ``  
