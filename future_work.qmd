...
query <- glue_sql("
CREATE TABLE first_path_point_1 AS
SELECT 
        seq,
        path_seq,
        start_vid,
  ...
  FROM pgr_dijkstra('
        ...
        FROM porto_alegre_net_largest',
                    ...
                    ARRAY(SELECT id AS end_id FROM hospital_rs_node_v2 WHERE ds_cnes = {hospital_name}),
                    directed:=TRUE) AS path
LEFT JOIN porto_alegre_net_largest AS net
...
# Send the query
result <- dbGetQuery(conn= eisenberg_connection, query)
# dijkstra_second_route

query_2 <- glue_sql("
CREATE TABLE second_path_point_1 AS
WITH pgr_doubled_dijkstra AS (
    SELECT 
        *
    FROM  pgr_dijkstra('
...
result <- dbGetQuery(conn= eisenberg_connection, query_2)

## Calculate teh second route
create_table_query <- glue_sql("
CREATE TABLE second_route_point_1 AS
SELECT 
    seq,
    path_seq,
    start_vid,
...
dbExecute(conn = eisenberg_connection, create_table_query)
select_query <- "SELECT * FROM second_route_point_1;"

# Fetch the data
result <- dbGetQuery(conn = eisenberg_connection, select_query)

# Third route
query_4 <- glue_sql("
CREATE TABLE third_path_point_1 AS
WITH pgr_doubled_dijkstra AS (
SELECT 
    *
FROM  pgr_dijkstra('
...
result <- dbGetQuery(conn = eisenberg_connection, query_4)

# third route
query_5 <- glue_sql("
CREATE TABLE third_route_point_1 AS
SELECT 
            seq,
            path_seq,
            start_vid,
...
CREATE TABLE three_alternative_routes_point_1 AS 
SELECT start_vid, end_vid,path, the_geom, edge FROM first_path_point_1
UNION
SELECT start_vid, end_vid,path, the_geom, edge FROM second_route_point_1
UNION
SELECT start_vid, end_vid,path, the_geom, edge FROM third_route_point_1", .con = eisenberg_connection)

result <- dbGetQuery(conn = eisenberg_connection, alterantive_point_1)

```

```{r}
table_end_point_1 <- glue_sql("
CREATE TABLE table_end_point_1 AS
SELECT
            alternative.*,
            ceil(st_length(alternative.the_geom::geography)/1000) As length,
            original.cost AS cost
          FROM
            three_alternative_routes_point_1 AS alternative
          LEFT JOIN
           porto_alegre_net_largest  AS original
          ON
          alternative.edge = original.id
          WHERE
            edge!= -1", .conn=eisenberg_connection)
result <- dbGetQuery(conn=eisenberg_connection,table_end_point_1 )
```
...
table_end <- glue_sql("CREATE TABLE table_end
                ...
                length float,
                cost float)")
result <- dbGetQuery(conn=eisenberg_connection,table_end )
```
...
calculations <- glue_sql("
        ...
        path
        ORDER BY path ASC")
dbGetQuery(conn=eisenberg_connection, calculations)
```
