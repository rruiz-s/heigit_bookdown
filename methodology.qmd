...
lapply(c("tidyverse","DT","leaflet","sf","htmltools", "DBI"),
       require,
       character.only =T)
```
...
## weighted sampling
od <- spatSample(pop_cropped, pop_ghs, "weights", as.points=TRUE, ) |> st_as_sf() |> st_transform(4326)
names(od)[1] <- 'building'
dbWriteTable(connection, c("hegit","od_77763"), od, append=T) 
DBI::dbWriteTable(connection, 
                  DBI::Id(schema = "heigit", table = "od_77763"), 
                  od)
```
