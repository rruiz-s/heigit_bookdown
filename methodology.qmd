...
After creating an ID for the samples, a neighbor nearest function snapped these points into the closest node of the network. Subsequently, 
...
LATERAL (SELECT
        * 
        ...
        LIMIT 1) AS net;
--- 1.2) Limiting to 100 origin samples in the pre flooding event
...
--- 1.3) Create 100 destination from the snapped points in the pre flooding event
